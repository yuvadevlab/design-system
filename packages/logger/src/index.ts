...
/**
 * Lightweight logger utility for Yuva DevLab applications.
 *
 ...
 * responses. Used by both HTTP servers and backend/frontend application services.
 */
import fs from "fs";
...
export function createLogger(config?: LoggerConfig): Logger {
  const enabled =
    config?.enabled ??
    (process.env.LOG_ENABLED ? process.env.LOG_ENABLED !== "false" : true);
  const level = config?.level ?? parseLogLevel(process.env.LOG_LEVEL);
  const file = config?.file ?? process.env.LOG_FILE;
  const fileStream =
    file && process.env.LOG_PERSIST === "true"
      ? createFileTransport(file)
      : null;
 ...
 *
 * Used across server apps so application services and the HTTP request
